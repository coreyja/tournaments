...
use super::{GameEngine, GameEvent, GameState, StandardEngine, Direction};
...
impl GameRunner {
    ...
    pub async fn run_game_with_random_moves(
        ...
        // Initialize game
        let mut game_state = self.engine.initialize_game(snake_ids.clone(), board_width, board_height);
        ...
        for turn in 0..max_turns {
            ...
            if self.engine.is_game_over(&game_state) {
                ...
                info!("Game {} ended at turn {} with winner: {:?}", game_id, turn, winner);
                break;
            }
        ...
        for (snake_id, _snake) in &game_state.snakes {
...
pub async fn run_and_store_game(
    ...
    // Run the game
    let result = runner.run_game_with_random_moves(
        game_id,
        snake_ids,
