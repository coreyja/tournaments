...
mod tests {
    ...
    fn test_snake_movement() {
        ...
        let initial_head = state.snakes.get(&snake_id).unwrap().get_head().clone();
        ...
        // Check events
        assert!(events.iter().any(|e| matches!(e, GameEvent::SnakeMoved { .. })));
    ...
    fn test_wall_collision() {
        ...
        assert!(events.iter().any(|e| matches!(e,
            GameEvent::SnakeDied { cause: DeathCause::WallCollision, .. }
        )));
    }
    ...
    fn test_food_consumption() {
        ...
        state.food.push(Food { position: Coordinate { x: 5, y: 6 } });
        ...
        assert!(!state.food.iter().any(|f| f.position == Coordinate { x: 5, y: 6 }));

        // Check food eaten event
        assert!(events.iter().any(|e| matches!(e,
            GameEvent::SnakeAteFood { .. }
        )));
    }
