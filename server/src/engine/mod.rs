...
pub use runner::{GameRunner, run_and_store_game};

// Main engine trait that different rule sets can implement
pub trait GameEngine {
    fn initialize_game(&self, snake_ids: Vec<Uuid>, board_width: u32, board_height: u32) -> GameState;
    fn process_turn(&self, state: &mut GameState, moves: HashMap<Uuid, Direction>) -> Vec<GameEvent>;
...
pub enum GameEvent {
    SnakeMoved { snake_id: Uuid, direction: Direction, new_head: Coordinate },
    SnakeAteFood { snake_id: Uuid, food_position: Coordinate },
    SnakeDied { snake_id: Uuid, cause: DeathCause },
    FoodSpawned { position: Coordinate },
...
impl GameEngine for StandardEngine {
    fn initialize_game(&self, snake_ids: Vec<Uuid>, board_width: u32, board_height: u32) -> GameState {
        ...
        let starting_positions = self.get_starting_positions(snake_ids.len(), board_width, board_height);
    ...
    fn get_winner(&self, state: &GameState) -> Option<Uuid> {
        let alive_snakes: Vec<_> = state.snakes
            .iter()
...
impl StandardEngine {
    fn get_starting_positions(&self, snake_count: usize, width: u32, height: u32) -> Vec<Coordinate> {
        // Simple starting positions - corners and edges
        let positions = vec![
            Coordinate { x: 1, y: 1 },                     // Bottom-left
            Coordinate { x: width - 2, y: height - 2 },    // Top-right
            Coordinate { x: 1, y: height - 2 },            // Top-left
    ...
    fn get_random_move(&self, snake: &Snake, board: &Board) -> Direction {
        ...
        valid_moves.choose(&mut rand::thread_rng()).copied()
            .unwrap_or(Direction::Up) // Fallback if no valid moves
    }

    fn check_collisions(&self, new_heads: &HashMap<Uuid, Coordinate>, state: &GameState) -> HashMap<Uuid, DeathCause> {
        ...
        for (snake_id, new_head) in new_heads {
            ...
            for (other_id, other_snake) in &state.snakes {
                ...
                if other_snake.body.contains(new_head) {
                    deaths.insert(*snake_id, DeathCause::SnakeCollision {
        ...
        for (snake_id, new_head) in new_heads {
            if !deaths.contains_key(snake_id) {
                head_collisions.entry(*new_head).or_default().push(*snake_id);
            }
        }
        ...
        for (_, snake_ids) in head_collisions {
            if snake_ids.len() > 1 {
                ...
                let mut sizes: Vec<_> = snake_ids.iter()
                ...
                for (snake_id, size) in &sizes {
                    if *size < max_size {
                        // Smaller snake dies
                        deaths.insert(**snake_id, DeathCause::HeadToHeadCollision {
